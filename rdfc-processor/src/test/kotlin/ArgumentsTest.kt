...
class ArgumentsTest {
  ...
  fun notSingle() {
    ...
    assertThrows<IllegalArgumentException> { args.get<String>("key") }
  }
  ...
  fun longListWrong() {
    ...
    assertThrows<IllegalArgumentException> { args.get<List<Int>>("key", strict = true) }
  }
  ...
  fun invalidCast() {
    val args = Arguments(mapOf("key" to listOf("value")))
    assertThrows<IllegalArgumentException> { args.get<Int>("key") }
  }
  ...
  fun pairs() {
    ...
    assertThrows<IllegalArgumentException> {
      args.get<Pair<Double, String>>("first", strict = true)
    }

    // Attempt to get string as integer.
    assertThrows<IllegalArgumentException> { args.get<Pair<Int, Int>>("first") }
  }
  ...
  fun inheritance() {
    ...
    assertThrows<IllegalArgumentException> { args.get<List<B>>("a", strict = true) }
    assertEquals(2, args.get<List<B>>("b", strict = true).size)
    assertEquals(2, args.get<List<B>>("c", strict = true).size)

    assertThrows<IllegalArgumentException> { args.get<List<C>>("a", strict = true) }
    assertThrows<IllegalArgumentException> { args.get<List<C>>("b", strict = true) }
